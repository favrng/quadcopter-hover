...
class Quadcopter_Roll_Random (gym.Env):

    def __init__(self):
        super().__init__() #import gym.env init
        print ("This is quadcopter roll only, random initial roll mode")
        #####################
        self.MAX_EPISODE = 1000 #1 episode = 1000 steps max
        #threshold untuk masing2 state
        #state ada 3: roll, roll dot, wx
        ...
        high = np.array([self.roll_threshold, self.roll_dot_threshold, self.wx_threshold],
                        dtype=np.float32)
        self.min_action = np.array([0.1, 0.1])
        self.max_action = np.array([20, 20]) #batas kp kd
        self.action_space = spaces.Box(self.min_action, self.max_action, shape = (2,), dtype = np.float32) #kp kd roll 

        self.observation_space = spaces.Box(-high, high, shape = (3,), dtype = np.float32) #roll, roll dot, wx
        
        ##########################
        self.steps_left = self.MAX_EPISODE
        #state array: roll, roll dot, wx -> used for RL training

        self.initial_roll, _ = self._generate_initial_value()
        self.state = [self.initial_roll, 0, 0] #roll, roll dot, wx
...
################## 
    def step(self, action): #main loop
        ...
        # tau_y = self.pd_controller(self.desired_pitch, self.pitch, self.desired_pitch_dot, self.pitch_dot, Kp_roll, Kd_roll)
        tau_y = 0
        ...
        # pd_pitch_error = self.desired_pitch - self.pitch
        self.pd_controller_error_values.append(self.pd_roll_error)

        self.pd_roll_dot_error = self.desired_roll_dot - self.roll_dot
        # pd_pitch_dot_error = self.desired_pitch_dot - self.pitch_dot
        self.pd_controller_dot_error_values.append(self.pd_roll_dot_error)
                
        ##################
        self.state = [self.roll, self.roll_dot, self.wx]
        ...
        self.reward = self.roll_reward #negative reward, orientasi roll 
    ...
    def reset(self):
        ...
        self.roll, self.pitch, self.yaw = self.initial_roll, 0, 0 #-0.5 sampai 0.5, step 0.1
        ...
        self.state = np.array ([self.roll, self.roll_dot, self.wx])
