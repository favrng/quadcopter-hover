...
class Quadcopter_Random (gym.Env):

    def __init__(self):
        super().__init__() #import gym.env init
        print ("This is quadcopter random initial roll and pitch")
        #####################
        self.MAX_EPISODE = 1000 #1 episode = 1000 steps max
        #threshold untuk masing2 state
        #state ada 6: roll, pitch, roll dot, pitch dot, wx, wy
        ...
        high = np.array([self.roll_threshold, self.pitch_threshold, self.roll_dot_threshold, self.pitch_dot_threshold, self.wx_threshold, self.wy_threshold],
                        dtype=np.float32)
        self.min_action = np.array([0.1, 0.1, 0.1, 0.1])
        self.max_action = np.array([20, 1, 20, 1]) #batas kp kd
        self.action_space = spaces.Box(self.min_action, self.max_action, shape = (4,), dtype = np.float32) #kp kd roll pitch 

        self.observation_space = spaces.Box(-high, high, shape = (6,), dtype = np.float32) #roll pitch, turunannya, omega x y
        
        ##########################
        self.steps_left = self.MAX_EPISODE

        #state array: roll, pitch, roll dot, pitch dot, wx, wy -> used for RL training

        self.initial_roll, self.initial_pitch = self._generate_initial_value()
        self.state = [self.initial_roll, self.initial_pitch, 0, 0, 0, 0] #roll, roll dot, wx
        # print("initial roll init:", self.state[0]) #debug
        #############################
...
################## 
    def step(self, action): #main loop
        ...
        # print("Action:", action)
        Kp_roll, Kd_roll, Kp_pitch, Kd_pitch = action
        ...
        tau_x = self.pd_controller(self.desired_roll, self.roll, self.desired_roll_dot, self.roll_dot, Kp_roll, Kd_roll)
        tau_y = self.pd_controller(self.desired_pitch, self.pitch, self.desired_pitch_dot, self.pitch_dot, Kp_pitch, Kd_pitch)
        tau_z = 0 #PD controller not needed
        ...
        self.pd_roll_error = self.desired_roll - self.roll
        self.pd_pitch_error = self.desired_pitch - self.pitch
        self.pd_controller_error_values.append([self.pd_roll_error, self.pd_pitch_error])

        self.pd_roll_dot_error = self.desired_roll_dot - self.roll_dot
        self.pd_pitch_dot_error = self.desired_pitch_dot - self.pitch_dot
        self.pd_controller_dot_error_values.append([self.pd_roll_dot_error, self.pd_pitch_dot_error])

        ##################
        self.state = [self.roll, self.pitch, self.roll_dot, self.pitch_dot, self.wx, self.wy]
        ...
        self.roll_reward = -np.square(self.desired_roll-self.state[0])
        self.pitch_reward = -np.square(self.desired_pitch-self.state[1])

        self.reward = self.roll_reward + self.pitch_reward #negative reward, orientasi roll 
    ...
    def reset(self):
        ...
        self.roll, self.pitch, self.yaw = self.initial_roll, self.initial_pitch, 0 #-0.5 sampai 0.5, step 0.1
        ...
        self.state = np.array ([self.roll, self.pitch, self.roll_dot, self.pitch_dot, self.wx, self.wy])
